...
if [ $# != 7 ]; then
   echo "Usage: local/train_deltas.sh <num-leaves> <tot-gauss> <data-dir> <lang-dir> <alignment-dir> <exp-dir> <subdata-dir>"
   echo "e.g.: local/train_deltas.sh 2000 10000 data/train_si84_half data/lang exp/mono_ali exp/tri1 data/train_clu3/train_clu0"
...
ssdata=$subdata/split$nj;
split_data.sh $subdata $nj || exit 1;
...
subfeats="ark,s,cs:apply-cmvn $cmvn_opts --utt2spk=ark:$ssdata/JOB/utt2spk scp:$ssdata/JOB/cmvn.scp scp:$ssdata/JOB/feats.scp ark:- | add-deltas $delta_opts ark:- ark:- |"
...
while [ $x -lt $tot_num_iters ]; do
  echo "$0: sub-training pass $x"
  if [ $stage -le $x ]; then
    if echo $realign_iters | grep -w $x >/dev/null; then
      echo "$0: sub-aligning data"
      mdl="gmm-boost-silence --boost=$boost_silence `cat $lang/phones/optional_silence.csl` $dir/$x.mdl - |"
      $cmd JOB=1:$nj $dir/log/align.$x.JOB.log \
        gmm-align-compiled $scale_opts --beam=$beam --retry-beam=$retry_beam --careful=$careful "$mdl" \
         "ark:gunzip -c $dir/fsts.JOB.gz|" "$subfeats" \
         "ark:|gzip -c >$dir/ali.JOB.gz" || exit 1;
    fi
    $cmd JOB=1:$nj $dir/log/acc.$x.JOB.log \
      gmm-acc-stats-ali  $dir/$x.mdl "$subfeats" \
       "ark,s,cs:gunzip -c $dir/ali.JOB.gz|" $dir/$x.JOB.acc || exit 1;
    $cmd $dir/log/update.$x.log \
      gmm-est --mix-up=$numgauss --power=$power \
        --write-occs=$dir/$[$x+1].occs $dir/$x.mdl \
